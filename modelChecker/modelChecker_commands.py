...
def run():
    print("✅ Model Checker Running!")

    # Example: Run all checks and print results
    functions_to_run = [
        duplicatedNames, namespaces, shapeNames, triangles, ngons, 
        noneManifoldEdges, openEdges, hardEdges, zeroAreaFaces
    ]
    
    for func in functions_to_run:
        try:
            result = func([], [])  # Modify as needed if input args are different
            print(f"✔️ {func.__name__} result: {result}")
        except Exception as e:
            print(f"⚠️ Error running {func.__name__}: {e}")